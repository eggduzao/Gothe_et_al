...
linePlot <- function(numberOfElements, listOfVectorsEto, listOfVectorsDmso, percentileLabels, outFileName){
  ...
  text(5, y = 9.5, paste("n = ",numberOfElements,sep=""), font = 3, cex = 1.2)
...
# Input
ifn = "/media/egg/cheops_agpapan/eduardo/Roukos_Bliss/Results/23_Final_CTCF_LOOP/4_CtcfLoopTranscription/"
cellList = c("K562", "TK6")
featureList = c("ctcf_loop_genes", "ctcf_loop_promoters")
motifList = c("TIO", "TO", "TI", "TF", "TR", "TFR", "restIN", "restOUT", "rest")
...
for(cell in cellList){
  for(feature in featureList){
    for(motif in motifList){
      ...
      if(motif == "restIN" | motif == "restOUT" | motif == "rest"){flagList = c(flagList, TRUE)}
      else{flagList = c(flagList, FALSE)}
      if(motif == "TIO"){flagAdj = c(flagAdj, 0.1286)}
      else if(motif == "TO"){flagAdj = c(flagAdj, 0)}
      else if(motif == "TI"){flagAdj = c(flagAdj, 0)}
      else if(motif == "TF"){flagAdj = c(flagAdj, 0.037)}
      else if(motif == "TR"){flagAdj = c(flagAdj, 0.041)}
      else if(motif == "TFR"){flagAdj = c(flagAdj, 0.1011)}
      else if(motif == "restIN"){flagAdj = c(flagAdj, 0.1331)}
      else if(motif == "restOUT"){flagAdj = c(flagAdj, 0.1295)}
      else if(motif == "rest"){flagAdj = c(flagAdj, 0.1286)}
    }
  }
}

# Parameters
percentileVec = c(200, 90, 50, 0)
percentileLabels = c("High GRO-seq", "Intermediate GRO-seq", "Low GRO-seq")
nElList = c(4611, 2317, 1421, 452, 421, 873, 1069, 2144, 3213,
            2134, 1238, 412, 285, 199, 484, 1069, 2144, 3213,
            5236, 2361, 1027, 1138, 710, 1848, 1382, 2972, 4360,
            1748, 1034, 524, 112, 78, 190, 1382, 2972, 4360)
...
for(i in 1:length(inFileEtoList)){
  ...
  if(flagList[i] == TRUE){
    ...
    # Heatmap Plot
    listOfRows = (inputTableEto[,11] < 200) & (inputTableEto[,11] >= 0)
    ...
    outFileName = outNameCorrList[i]
    pAdj = 0
    scatterPlot(vec1, vec2, xLabel, ylabel, flagAdj[i], outFileName)
