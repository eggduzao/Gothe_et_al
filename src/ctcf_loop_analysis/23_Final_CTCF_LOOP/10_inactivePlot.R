...
linePlot4 <- function(numberOfElements, listOfVectorsEto, listOfVectorsDmso, outFileName){
  ...
  # Axis
  axis(side = 1, at = c(1,51,100), labels = c("-500", "CTCF", "+500"))
  axis(side = 2, at = seq(from = 0.0, to = 8, by = 1))
...
scatterPlot <- function(vec1, vec2, xLabel, ylabel, pAdj, outFileName){
  ...
  pplot = pplot + scale_fill_gradientn(colours = myPalette(11), limits=c(0.0,4.0))
...
# Input
ifn = "/media/egg/cheops_agpapan/eduardo/Roukos_Bliss/Results/23_Final_CTCF_LOOP/10_inactivePlot/"
cellList = c("K562", "TK6")
featureList = c("ctcf_inactive_genes", "ctcf_inactive_promoters")
motifList = c("MFR_TFR", "MFR_TF", "MFR_TR", "MF_TFR", "MR_TFR", "MFTF_MRTR", "MFTR_MRTF")
...
for(cell in cellList){
  for(feature in featureList){
    for(motif in motifList){
      ...
      inFileDmsoList = c(inFileDmsoList, paste(ifn,cell,"_ETO_",feature,"_",motif,".txt",sep=""))
...
for(i in 1:length(inFileEtoList)){
  ...
  numberOfElements = nrow(inputTableEto)
  ...
  if(flagList[i] == TRUE){
    ...
    # Heatmap Plot
    listOfRows = (inputTableEto[,11] < 200) & (inputTableEto[,11] >= 50)
