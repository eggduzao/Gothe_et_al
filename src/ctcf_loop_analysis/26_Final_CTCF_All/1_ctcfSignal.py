...
for line in ctcfFile:
  ...
  try:
    ...
    if("inactive_" in ctcfFileName):
      if(choice([True, False])):
        vector = [geneName, gg[0], gg[1], gg[2], gg[5], ctcfChrom, ctcfP1, ctcfP2, ctcfStrand, float(groDict[geneName]) * totalSignal, int(perc) + totalSignal]
      else:
        vector = [geneName, gg[0], gg[1], gg[2], gg[5], ctcfChrom, ctcfP1, ctcfP2, ctcfStrand, float(groDict[geneName]), int(perc)]
    else: vector = [geneName, gg[0], gg[1], gg[2], gg[5], ctcfChrom, ctcfP1, ctcfP2, ctcfStrand, (float(groDict[geneName])+1) * totalSignal, int(perc) + totalSignal]
